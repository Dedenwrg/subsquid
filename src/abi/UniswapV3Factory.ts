...
import {ABI_JSON} from './UniswapV3Factory.abi'
...
export const functions = {
    ...
    parameters: new Func<[], {}, ([factory: string, token0: string, token1: string, fee: number, tickSpacing: number] & {factory: string, token0: string, token1: string, fee: number, tickSpacing: number})>(
        abi, '0x89035730'
    ),
...
export class Contract extends ContractBase {
    ...
    parameters(): Promise<([factory: string, token0: string, token1: string, fee: number, tickSpacing: number] & {factory: string, token0: string, token1: string, fee: number, tickSpacing: number})> {
